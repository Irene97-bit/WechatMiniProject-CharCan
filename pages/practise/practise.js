...
Page({
  ...
  create(){
    ...
    console.log(this.data.hsk1word[0].hanzi)
    this.setData({
      svg : buildSvg(this.data.case)
    })
  },

  reStartAnimation() {
    this.setData({
      svgBase64: this.toUniqueBase64(this.data.svg)
    })
  },
toUniqueBase64(svg) {
  const uniqueMark = `<!-- unique mark: ${Date.now()} -->\n`
  return `data:image/svg+xml;base64,${Base64.encode(uniqueMark + svg)}`
},
...
tapSvg() {
  ...
  onLoad: function (options) {
    ...
    wx.request({
      ...
      success: res => {
        ...
        this.setData({
          hsk1word : [res.data.data[index]],
          index:index,
          char : res.data.data[index].hanzi
        })
        console.log(res.data.data[index].hanzi)
        this.getsvgjson()
        this.create()
        this.reStartAnimation()
      } 
  ...
  getsvgjson: function(options){
        var VERSION = '2.0';
        wx.request({
          url: 'https://cdn.jsdelivr.net/npm/hanzi-writer-data@' + VERSION + '/' + encodeURIComponent(this.data.char) + '.json',//json数据地址
          headers: {
            'Content-Type': 'application/json'
          },     
          success: res1 => {
            ...
            this.setData({
              case:{
                strokes: res1.data.strokes,
                medians: res1.data.medians
              }
            })
      } 
    })
  ...
  setCanvasBg(color) {
    ...
    this.data.ctx.setFillStyle('lightgrey')
    this.data.ctx.beginPath()
    this.data.ctx.setStrokeStyle('red')
    this.data.ctx.setFontSize(200)
    this.data.ctx.fillText(this.data.hsk1word[0].hanzi, 50, 220)
    this.data.ctx.moveTo(0, 150)//初始点选择
    this.data.ctx.lineTo(300, 150)//画线
    this.data.ctx.moveTo(150, 0)//初始点选择
    this.data.ctx.lineTo(150, 300)//画线
    this.data.ctx.moveTo(0, 0)//初始点选择
    this.data.ctx.lineTo(300, 300)//画线
    this.data.ctx.moveTo(300, 0)//初始点选择
    this.data.ctx.lineTo(0, 300)//画线
